<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <!-- Table -->
      <div class="bg-white rounded-2xl border border-gray-200 overflow-x-auto animate-fade-up" style="animation-delay: 60ms">
        <table class="w-full text-sm min-w-[640px]">
          ...
          <tbody>
            <tr
              v-for="msg in messagesStore.items"
              :key="msg.id"
              class="border-b border-gray-50 hover:bg-gray-50 transition-colors"
            >
              <td class="px-4 py-3">
                <span class="font-medium text-gray-900">{{ msg.user.first_name || msg.user.username || 'Unknown' }}</span>
              </td>
        ...
        <div v-if="!messagesStore.loading && messagesStore.items.length === 0" class="text-center py-8 text-gray-400">
          Murojaatlar topilmadi
        </div>
      </div>

      <!-- Pagination -->
      <div class="flex items-center justify-between animate-fade-up" style="animation-delay: 90ms" v-if="messagesStore.total > 0">
...
// If date_from is set after date_to, clear date_to
...
// Load when page changes (but not from filter reset above)
